"""Reserve Bank of Australia — Statement on Monetary Policy extractor (Phase 4.x).

The RBA's quarterly **Statement on Monetary Policy** (SMP) is a large narrative
publication: it sets out the outlook for economic growth (GDP), the labour
market, inflation, financial market conditions, the international environment,
risks to the outlook and the monetary policy stance.
...
_IGNORE_HEADINGS = frozenset({
    "statement on monetary policy", "contents", "foreword", "editorial",
    "glossary", "references", "bibliography", "annex", "appendix", "methodology",
...
_POLICY_HEADINGS = frozenset({
    ...
    "the stance of monetary policy", "stance of monetary policy", "monetary policy decisions",
})
_RISK_HEADINGS = frozenset({
    "risks", "risk assessment", "risks to the outlook", "risks and uncertainties",
    "the risks to the outlook", "risks and scenarios", "scenarios",
})
_INFLATION_HEADINGS = frozenset({
    "inflation", "inflation and prices", "prices costs and inflation",
    "costs and inflation", "inflation outlook", "prices and costs",
})
_LABOUR_HEADINGS = frozenset({
    "the labour market", "labour market", "employment", "wages", "unemployment",
})
_FINANCIAL_HEADINGS = frozenset({
    "financial market conditions", "financial conditions", "financial markets",
    "monetary and financial conditions", "money and credit", "financial stability",
})
_GROWTH_HEADINGS = frozenset({
    "economic outlook", "domestic economic conditions", "domestic demand and production",
    "the domestic economy", "domestic economy", "output and demand", "economic activity", "gdp",
    "the international environment", "international environment", "the world economy",
    "world economy", "global economy",
    "the international economy", "international economy", "economic growth",
})
_GENERAL_HEADINGS = frozenset({
    "overview", "summary", "executive summary", "introduction", "economic analysis",
})
...
_POLICY_TERM = re.compile(
    r"\b(?:monetary\s+policy|policy\s+rate|cash\s+rate|interest\s+rates?|"
    r"reserve\s+bank\s+of\s+australia|monetary\s+policy\s+board)\b",
    re.IGNORECASE,
)
...
_INFLATION_ANCHORS = (
    ...
    re.compile(r"\b(?:trimmed\s+mean|weighted\s+median)\b", re.IGNORECASE),
...
_GROWTH_ANCHORS = (
    ...
    re.compile(r"\bservices\s+export(?:s)?\b", re.IGNORECASE),
...
class RbaReportExtractor(ReportsExtractor):
    bank = "rba"
    ...
    def extract(self, publication, document: NormalizedDocument) -> ExtractionResult:
        ...
        reporter = Reporter(extraction_version=EXTRACTION_VERSION, bank_tag="rba")
        risk_found = False
