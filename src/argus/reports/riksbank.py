"""Sveriges Riksbank — Monetary Policy Report extractor (Phase 4.x).

The Riksbank's quarterly **Monetary Policy Report** (MPR) is a large narrative
publication: the economic outlook, CPIF inflation, the labour market, financial
conditions, risks and alternative scenarios, and the monetary policy analysis
...
with the other Report extractors:

- sections are routed deterministically by normalized heading — known economic
  headings are mined (content-first), while the report masthead, the known
...
EXTRACTION_VERSION = "10.6.0"

# ``core_inflation`` is the canonical Phase 4.x subject for underlying / core
# inflation measures; the Riksbank's "underlying inflation" (CPIF excluding
# energy) is its core measure, distinct from the headline CPIF (→ ``inflation``).
...
# The Riksbank writes section titles with decorative dash variants
# ("Monetary policy in Sweden — the Riksbank's strategy"); dashes in headings
# are normalized to a space (structural, bank-local) so heading identity never
# depends on the exact dash glyph.
_DASH = re.compile(r"[\u2010\u2011\u2012\u2013\u2014\u2015\u2212-]+")

_IGNORE_HEADINGS = frozenset({
    "monetary policy report", "contents", "foreword", "editorial", "summary box",
    "list of boxes", "list of tables", "glossary", "references", "bibliography",
    "annex", "appendix", "methodology", "legal notice", "disclaimer",
    "acknowledgements", "notes", "statistics",
    "forecast tables", "the forecast tables",
})
_POLICY_HEADINGS = frozenset({
    "monetary policy", "monetary policy stance", "policy stance",
    "monetary policy analysis", "monetary policy assessment",
    "the monetary policy assessment", "policy considerations",
    "monetary policy decisions", "the policy rate path", "policy rate path",
    "monetary policy in sweden",
    "monetary policy in sweden the riksbank's strategy",
    "monetary policy in sweden the riksbanks strategy",
})
_RISK_HEADINGS = frozenset({
    "risks", "risk assessment", "uncertainty", "risks and uncertainty",
    "uncertainty and risks", "risks to the outlook", "the risks to the outlook",
    "risks and alternative scenarios", "uncertainty risks and alternative scenarios",
    "uncertainty, risks and alternative scenarios",
})
_INFLATION_HEADINGS = frozenset({
    "inflation", "inflation in sweden", "inflation expectations", "cpif",
    "cpif inflation", "price developments", "inflation and costs",
    "prices and costs", "costs and prices", "inflation outlook",
})
_LABOUR_HEADINGS = frozenset({
    "the labour market", "labour market", "employment", "wages",
    "wage developments", "unemployment",
})
_FINANCIAL_HEADINGS = frozenset({
    "financial conditions", "financial markets", "financial developments",
    "financial stability", "monetary conditions", "money and credit",
})
_GROWTH_HEADINGS = frozenset({
    "the economic outlook", "economic outlook",
    "the economic outlook for the coming years", "economic outlook for the coming years",
    "the outlook for the coming years", "economic activity",
    "the swedish economy", "swedish economy", "the economy", "growth", "output",
    "development of demand", "demand and output", "international developments",
    "the international economy", "international economy",
})
_FISCAL_HEADINGS = frozenset({
    "fiscal policy", "fiscal developments", "public finances", "fiscal",
})
_GENERAL_HEADINGS = frozenset({
    "summary", "executive summary", "overview", "introduction",
})
...
_GUIDANCE_ANCHORS = (
    ...
    re.compile(r"\b(?:the\s+)?(?:executive\s+board\s+)?will\s+make\s+its\s+decisions\s+(?:on\s+the\s+basis\s+of|based\s+on)\b", re.IGNORECASE),
...
_POLICY_TERM = re.compile(
    r"\b(?:monetary\s+policy|policy\s+rates?|policy\s+rate\s+path|interest\s+rates?|"
    r"executive\s+board|policy\s+instruments?|monetary\s+policy\s+stance)\b",
    re.IGNORECASE,
)
_POLICY_STANCE = re.compile(
    r"\bdecided\s+to\b|\bdecision(?:s)?\b|\bstance\b|\bappropriate\b|\brestrictive\b|"
    r"\baccommodative\b|\btighten(?:ed|ing)?\b|\beas(?:ed|ing)?\b|\bcontractionary\b|"
    r"\bexpansionary\b|\bneutral\b|\b(?:cut|lowered|raised|changed)\s+the\s+policy\s+rate\b|"
    r"\bheld\s+the\s+policy\s+rate\b",
    re.IGNORECASE,
)
_RISK_ANCHORS = (
...
_INFLATION_ANCHORS = (
    ...
    re.compile(r"\bcpif\b", re.IGNORECASE),
...
class RiksbankReportExtractor(ReportsExtractor):
    bank = "riksbank"
    ...
    def extract(self, publication, document: NormalizedDocument) -> ExtractionResult:
        ...
        reporter = Reporter(extraction_version=EXTRACTION_VERSION, bank_tag="riksbank")
        risk_found = False
        ...
        def categorize(sentence: str) -> str:
            ...
            if matches(_GROWTH_ANCHORS, sentence):
                return CAT_GROWTH
            if matches(_FISCAL_ANCHORS, sentence):
                return CAT_FISCAL
            return "none"
        ...
        for index, section in enumerate(document.sections):
            ...
            for sentence in split_sentences(section.text or ""):
                ...
                if cat == CAT_GROWTH:
                    ...
                    continue
                if cat == CAT_FISCAL:
                    reporter.emit_text(result, document, index, sentence, SUBJECT_FISCAL_POLICY, "assessment")
                    continue
    ...
    def _risk_subject(sentence: str) -> str:
        ...
        if "inflation" in lower or "prices" in lower or "cpif" in lower:
    ...
    def _inflation(result, reporter, document, index, sentence) -> None:
        ...
        elif "underlying inflation" in lower or "excluding energy" in lower:
            subject = SUBJECT_CORE_INFLATION
        elif "cpif" in lower or "inflation" in lower:
