"""Reserve Bank of New Zealand — Monetary Policy Statement extractor (Phase 4.x).

The RBNZ's quarterly **Monetary Policy Statement** (MPS) is a large narrative
publication: it sets out the outlook for the New Zealand economy (GDP), the
...
_IGNORE_HEADINGS = frozenset({
    "monetary policy statement", "contents", "foreword", "editorial",
...
_POLICY_HEADINGS = frozenset({
    ...
    "monetary policy setting", "the policy path", "policy path", "the ocr",
    "monetary policy decisions",
})
_RISK_HEADINGS = frozenset({
    "risks", "risk assessment", "risks to the outlook", "the risks to the outlook",
    "risks and uncertainties", "the risks",
})
_INFLATION_HEADINGS = frozenset({
    "inflation", "inflation outlook", "prices and costs", "inflation and prices",
    "costs and prices",
})
...
_FINANCIAL_HEADINGS = frozenset({
    "financial conditions", "financial markets", "financial system",
    "monetary conditions", "money and credit", "financial stability",
})
_GROWTH_HEADINGS = frozenset({
    "the economic outlook", "economic outlook", "the outlook", "gdp",
    "economic activity", "the new zealand economy", "new zealand economy", "output",
    "the global economy", "global economy", "the international economy", "international economy",
    "the international environment", "international environment",
...
_POLICY_TERM = re.compile(
    r"\b(?:monetary\s+policy|policy\s+rates?|ocr|official\s+cash\s+rate|interest\s+rates?|"
    r"monetary\s+policy\s+committee|mpc|reserve\s+bank\s+of\s+new\s+zealand)\b",
    re.IGNORECASE,
)
...
class RbnzReportExtractor(ReportsExtractor):
    bank = "rbnz"
    ...
    def extract(self, publication, document: NormalizedDocument) -> ExtractionResult:
        ...
        reporter = Reporter(extraction_version=EXTRACTION_VERSION, bank_tag="rbnz")
        risk_found = False
