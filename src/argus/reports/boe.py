"""Bank of England — Monetary Policy Report extractor (Phase 4.x).

The BoE's quarterly **Monetary Policy Report** (formerly the Inflation Report)
is a large narrative publication: it sets out the MPC's assessment of the
...
- sections are routed deterministically by normalized heading — a known
  economic heading is mined (content-first), while the report masthead and the
  known non-economic headings (contents, glossary, annexes, methodology,
  boxes, …) and **unknown** headings are ignored (`UNKNOWN ≠ ECONOMIC`);
- sentence classification is content-first with a fixed precedence:
  guidance > policy > risk > financial > inflation > labour > growth > fiscal;
- a quantitative value Fact is produced only behind an explicit value-claim
  verb with a percentage and, for a forecast, an explicit reference year;
...
_IGNORE_HEADINGS = frozenset({
    "monetary policy report", "inflation report", "contents", "foreword",
    "editorial", "summary box", "list of boxes", "list of tables", "glossary",
    "references", "bibliography", "annex", "appendix", "methodology",
    "legal notice", "disclaimer", "acknowledgements", "notes",
})
_POLICY_HEADINGS = frozenset({
    "monetary policy", "monetary policy stance", "policy stance",
    "the policy decision", "the bank rate decision", "monetary policy decisions",
})
_RISK_HEADINGS = frozenset({
    "risks", "risk assessment", "the outlook and risks", "risks to the outlook",
    "risks and the outlook", "uncertainty",
})
_INFLATION_HEADINGS = frozenset({
    "inflation", "supply costs and prices", "costs and prices", "prices and costs",
    "inflation and costs", "cpi inflation", "inflation outlook",
})
_LABOUR_HEADINGS = frozenset({
    "the labour market", "labour market", "employment", "wages", "unemployment",
})
_FINANCIAL_HEADINGS = frozenset({
    "money credit and finance", "financial conditions", "financial markets",
    "money and credit", "the financial system", "financial developments",
})
_GROWTH_HEADINGS = frozenset({
    "demand and output", "the economic outlook", "economic outlook", "output and demand",
    "the uk economy", "uk economy", "economic activity", "economic growth", "growth",
})
_FISCAL_HEADINGS = frozenset({"fiscal policy", "the fiscal position", "fiscal position", "fiscal"})
_GENERAL_HEADINGS = frozenset({
    "summary", "executive summary", "overview", "introduction",
    "the world economy", "global economy", "international economy",
    "the international environment", "economic analysis",
})
...
_POLICY_TERM = re.compile(
    r"\b(?:monetary\s+policy|policy\s+rates?|bank\s+rate|interest\s+rates?|monetary\s+policy\s+committee|"
    r"mpc|committee)\b",
    re.IGNORECASE,
)
...
class BoeReportExtractor(ReportsExtractor):
    bank = "boe"
    ...
    def extract(self, publication, document: NormalizedDocument) -> ExtractionResult:
        ...
        reporter = Reporter(extraction_version=EXTRACTION_VERSION, bank_tag="boe")
        risk_found = False
        ...
        def categorize(sentence: str) -> str:
            if matches(_GUIDANCE_ANCHORS, sentence):
                return CAT_POLICY  # guidance handled as policy at emission below
        ...
        for index, section in enumerate(document.sections):
            ...
            for sentence in split_sentences(section.text or ""):
                ...
                if cat == CAT_RISK:
                    ...
                    reporter.emit_risk(result, document, index, sentence, subject)
                    continue
                if cat == CAT_POLICY:
                    if matches(_GUIDANCE_ANCHORS, sentence):
                        guidance_found = True
