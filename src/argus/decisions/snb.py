"""SNB — Swiss National Bank monetary policy assessment extractor.

Extracts the facts of an SNB monetary policy assessment from the normalized
document, answering "what did the SNB explicitly decide or announce in its
assessment?":

- the decision date (the "as of <date>" in the heading / leading paragraph)
- the SNB policy rate level — a single policy rate, percentage
- explicit changes ("is lowering the SNB policy rate by 25 basis points to
  1.25%") — sign preserved as basis points; a hold ("is keeping … at 0.50%")
  never fabricates a delta
- the decision wording ("The Swiss National Bank is lowering the SNB policy
  rate by 25 basis points to 1.25%."), verbatim
...
- votes, hawkish/dovish interpretation, forex — never
...
# "19 June 2026" in "Monetary policy assessment as of 19 June 2026".
_DATE_TOKEN = re.compile(rf"\b[0-9]{{1,2}}\s+(?:{_MONTH_WORDS})\s+[0-9]{{4}}\b", re.IGNORECASE)

# Level: "… keeping the SNB policy rate at 0.50%", "… lowering … to 1.25%".
_LEVEL = re.compile(
    rf"(?:snb\s+policy\s+rate|policy\s+rate)\b[^.]*?\b(?:to|at)\s+(?P<token>{_RATE_ITEM}\s*%)",
    re.IGNORECASE,
)

_DIRECTIONAL = r"(?:lower|decrease|reduce|cut|drop|ease|increase|raise|hike|lift)"
_CHANGE = re.compile(
    rf"\b(?P<verb>{_DIRECTIONAL})\w*\s+(?:the\s+)?(?:snb\s+policy\s+rate|policy\s+rate)\b[^.]*?\bby\s+"
    rf"(?P<amount>{_RATE_ITEM}\s*basis\s+points?)",
    re.IGNORECASE,
)

_DECISION_STATEMENT = re.compile(r"\b(?:is\s+)?(?:lowering|raising|increasing|decreasing|reducing|keeping|leaving|maintaining)\b", re.IGNORECASE)
_DECISION_CONTEXT = re.compile(r"\b(?:snb\s+policy\s+rate|policy\s+rate|swiss\s+national\s+bank)\b", re.IGNORECASE)

_GUIDANCE_ANCHORS: tuple[re.Pattern, ...] = (
    re.compile(r"\bconditional\s+on\b", re.IGNORECASE),
    re.compile(r"\bstands?\s+ready\s+to\s+adjust\b", re.IGNORECASE),
    re.compile(r"\bwill\s+continue\s+to\b", re.IGNORECASE),
    re.compile(r"\bwill\s+monitor\b", re.IGNORECASE),
)
...
class SnbDecisionExtractor(DecisionExtractor):
    bank = "snb"
    ...
    def _decision_date(document) -> tuple[str, str, int] | None:
        for index, section in enumerate(document.sections):
            ...
            if "as of" in heading.lower():
                match = _DATE_TOKEN.search(heading)
                if match:
    ...
    def _decision_wordings(document) -> list[tuple[int, int, str]]:
        ...
        for index, section in enumerate(document.sections):
            if normalize_title(section.heading or "") in ("economic conditions", "inflation outlook", "monetary conditions"):
                continue
