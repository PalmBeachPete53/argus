"""RIKSBANK — Sveriges Riksbank policy rate decision extractor.

Extracts the facts of a Riksbank policy rate decision from the normalized press
release, answering "what did the Executive Board explicitly decide?":

- the decision date (the leading release-date line / "At its meeting on …")
- the policy rate level — a single rate, percentage
- explicit changes ("lower the policy rate by 0.25 percentage points to 4.00
  per cent") — sign preserved as basis points (0.25 pp ≡ 25 bps)
...
- "percentage points" are converted to basis points only for the canonical
  ``change`` value kind; the source wording is preserved verbatim.
...
# "25 June 2026" (day-month-year).
_DATE_TOKEN = re.compile(rf"\b[0-9]{{1,2}}\s+(?:{_MONTH_WORDS})\s+[0-9]{{4}}\b", re.IGNORECASE)

# Level: "… lower the policy rate by 0.25 percentage points to 4.00 per cent".
_LEVEL = re.compile(
...
_DECISION_STATEMENT = re.compile(r"\b(?:decided|announced|agreed)\s+(?:today\s+)?(?:to|that)\b", re.IGNORECASE)
_DECISION_CONTEXT = re.compile(r"\bpolicy\s+rate\b|\bexecutive\s+board\b|\bmonetary\s+policy\b|\briksbank\b", re.IGNORECASE)

_GUIDANCE_ANCHORS: tuple[re.Pattern, ...] = (
    re.compile(r"\bpolicy\s+rate\s+is\s+expected\b", re.IGNORECASE),
    re.compile(r"\bwill\s+continue\s+to\s+(?:cut|lower|adjust|monitor)\b", re.IGNORECASE),
    re.compile(r"\bstands?\s+ready\s+to\s+adjust\b", re.IGNORECASE),
    re.compile(r"\b(the\s+)?policy\s+rate\s+can\s+(?:also\s+)?be\s+adjusted\b", re.IGNORECASE),
    re.compile(r"\bwill\s+be\s+cautious\b", re.IGNORECASE),
)
...
class RiksbankDecisionExtractor(DecisionExtractor):
    bank = "riksbank"
    ...
    def _decision_date(document) -> tuple[str, str, int] | None:
        for index, section in enumerate(document.sections):
            ...
            match = re.search(r"\bmeeting\s+on\s+", text, re.IGNORECASE)
            if match:
                date_match = _DATE_TOKEN.search(text[match.end() :])
                if date_match:
                    dt = parse_datetime(date_match.group(0))
                    if dt is not None:
                        return dt.date().isoformat(), date_match.group(0), index
        for index, section in enumerate(document.sections):
